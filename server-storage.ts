...
import { db } from "./db";
import { pool } from "./db";
import { and, asc, count, desc, eq, exists, gte, lte, ne, not, or, sql } from "drizzle-orm";
...
export class DatabaseStorage implements IStorage {
  ...
  constructor() {
    // Use PostgresSessionStore for persistent sessions across restarts
    try {
      ...
      // Ensure the session store has access to the pool for WebSocket diagnostics
      (this.sessionStore as any).pool = pool;
      
      console.log("PostgreSQL session store initialized successfully with pool access");
    } catch (error) {
      console.error("Failed to initialize PostgreSQL session store:", error);
      // Fallback to MemoryStore if PostgreSQL session store fails
    ...
    // Check if admin user exists, if not create with already hashed password
    this.getUserByUsername("admin").then(user => {
      if (!user) {
        console.log("Creating admin user...");
        // The password is 'password123' but pre-hashed
        this.createUser({
          username: "admin",
          // This is a pre-hashed version of 'password123'
          password: "2e85e006b862f2aad00cd784a4a98dc99b7860c324faf2079d8cd16d2db0a18aa5ca8419d4d5815119616b6818b545e4e65ff823eea30b6810c0e7bc5b229a0b.d213ef640ff58d7846cd05060e90f57e",
  ...
  async saveDriverRegistrationProgress(userId: number, progress: { step: number, formData: any, vehicleData: any }): Promise<any> {
    try {
  ...
  async deleteRide(id: number): Promise<boolean> {
    try {
      // First, delete any bids associated with this ride
      await db.delete(bids).where(eq(bids.rideId, id));
  ...
  async getBidsByDriver(driverId: number): Promise<Bid[]> {
    try {
      ...
      console.log(`Found ${result.length} bids for driver ${driverId}`);
      return result;
    } catch (error) {
  ...
  async createBid(bid: InsertBid): Promise<Bid> {
    try {
      ...
      // Check counter offer limits for the ride
      const [rideCounterOffers] = await db
        .select({ count: count() })
        .from(bids)
        .where(and(
          eq(bids.rideId, bid.rideId),
          eq(bids.status, "countered")
        ));
      
      // If this is a counter offer, check limits
      if (bid.parentBidId) {
        const maxCounterOffers = bid.maxCounterOffers || 3;
        if (rideCounterOffers.count >= maxCounterOffers) {
          throw new Error(`Maximum counter offers reached for this ride (${maxCounterOffers})`);
        }
      }
  ...
  async getDriverDetailsByUserId(userId: number): Promise<DriverDetails | undefined> {
    try {
      ...
      console.error("Error getting driver details by user ID:", error);
      return undefined;
    }
  }
